...
Page({
	...
	onLoad: function (options) {
		that = this;
    var sellerId = options.id;
    wx.setStorageSync('sellerId', sellerId);
    that.loadSeller(sellerId);
    that.loadCategory(sellerId);
    that.loadFood(undefined,sellerId);
	},
  ...
  loadCategory: function (sellerId) {
  ...
  loadFood: function (category, sellerId) {
    ...
    that.loadFood(category, sellerId);
	},
	checkout: function () {
		// 将对象序列化
		var cartObjects = [];
    ...
    var sellerId = wx.getStorageSync('sellerId');
		wx.navigateTo({
      url: '../../order/checkout/checkout?quantity=' + that.data.quantity + '&amount=' + that.data.amount + '&sellerId=' + sellerId+'&express_fee=' + that.data.express_fee + '&carts=' + JSON.stringify(cartObjects)
		});
	},
	add: function (e) {
		// 所点商品id
