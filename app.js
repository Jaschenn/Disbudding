...
App({
    ...
    if (Bmob.User.current()) {
		...
		wx.login({
          ...
          query.get(user.id, {
              ...
              wx.setStorageSync('sellerId', result.get("sellerId"));
            },
  ...
  loadAllSeller: function (cb) {
	...
	payment: function (order) {
		...
		//传参数金额，名称，描述,openid
		Bmob.Pay.wechatPay(order.get('total'), '灵犀微店', '订餐', openId).then(function (resp) {
			console.log('resp');
			console.log(resp);
			//服务端返回成功
